...
/**
 * Constructor for the ROS-independent algorithm object.
*/
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::msg::LaserScan& scan)
{
  ...
  const double scan_period = (current_scan_time - last_odom_time).seconds();
  if (!std::isfinite(scan_period) || scan_period <= 0.0)
  {
    RCLCPP_WARN(logger_, "Rejecting scan with non-increasing timestamp");
    return false;
  }
  fps = static_cast<float>(1.0 / scan_period);
  ...
  const auto runtime_ms =
    std::chrono::duration<double, std::milli>(runtime).count();
  RCLCPP_DEBUG(logger_, "execution time (ms): %f", runtime_ms);
