...
#include "robot_rviz_plugins/nav2_panel.hpp"
...
using nav2_rviz_plugins::GoalUpdater;
using nav2_rviz_plugins::QActionState;
using nav2_rviz_plugins::ROSActionQEvent;
using nav2_rviz_plugins::ROSActionQTransition;

Nav2Panel::Nav2Panel(QWidget * parent)
: Panel(parent),
  server_timeout_(100)
{
  async_tasks_.setCancelOnWait(true);
...
Nav2Panel::~Nav2Panel()
{
  QObject::disconnect(&GoalUpdater, nullptr, this, nullptr);
  timer_.stop();
  state_machine_.stop();

  if (initial_thread_) {
    auto * initial_thread = initial_thread_.data();
    initial_thread->requestInterruption();
    // is_active() is bounded to one second.  Waiting here guarantees that no
    // QThread retains lifecycle clients while RViz destroys its ROS context.
    initial_thread->wait();
    delete initial_thread;
  }

  async_tasks_.waitForFinished();
}
...
bool Nav2Panel::parseLoopValue(std::string & loop_value, int & value)
{
  if (!isLoopValueValid(loop_value)) {
    return false;
  }
  try {
    value = stoi(loop_value);
...
void Nav2Panel::loophandler()
{
  ...
  int parsed_loop_value = 0;

  // Sanity check for the loop value
  if (!parseLoopValue(loop_no_, parsed_loop_value)) {
    return;
  }
...
Nav2Panel::onInitialize()
{
  ...
  navigation_feedback_sub_ =
    ...
    [this](const nav2_msgs::action::NavigateToPose::Impl::FeedbackMessage::SharedPtr msg) {
      auto loop_value = nr_of_loops_->displayText().toStdString();
      int parsed_loop_value = 0;
      if (parseLoopValue(loop_value, parsed_loop_value) && parsed_loop_value > 0) {
    ...
    [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
      if (msg->status_list.empty()) {
        navigation_goal_status_indicator_->setText(
          nav2_rviz_plugins::getGoalStatusLabel(
            "Feedback", action_msgs::msg::GoalStatus::STATUS_UNKNOWN));
        return;
      }
      const auto status = msg->status_list.back().status;
      navigation_goal_status_indicator_->setText(
        nav2_rviz_plugins::getGoalStatusLabel("Feedback", status));
      auto loop_value = nr_of_loops_->displayText().toStdString();
      int parsed_loop_value = 0;
      const bool loop_value_valid = parseLoopValue(loop_value, parsed_loop_value);
      // Clearing all the stored values once reaching the final goal
      if (
        loop_value_valid && loop_count_ == parsed_loop_value &&
...
void Nav2Panel::onResumedWp()
{
  ...
  auto loop_value = nr_of_loops_->displayText().toStdString();
  int parsed_loop_value = 0;
  if (!parseLoopValue(loop_value, parsed_loop_value)) {
    return;
  }
  loop_no_ = std::to_string(parsed_loop_value - loop_count_);
  waypoint_status_indicator_->setText(
  ...
  try {
    ...
    if (rclcpp::ok()) {
      RCLCPP_WARN(
        ...
        "Stopping Nav2 panel timer after executor error: %s", error.what());
    }
    return false;
  }
  return true;
...
Nav2Panel::startWaypointFollowing(std::vector<geometry_msgs::msg::PoseStamped> poses)
{
  ...
  waypoint_follower_goal_.number_of_loops = parsed_loop_value;
...
PLUGINLIB_EXPORT_CLASS(robot_rviz_plugins::Nav2Panel, rviz_common::Panel)
