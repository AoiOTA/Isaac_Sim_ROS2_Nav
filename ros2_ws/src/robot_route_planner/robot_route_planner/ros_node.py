...
class RouteCoordinator:
    def __init__(self, node) -> None:
        ...
        self.reset_status_authority_seen = False
        self.reset_hold_barrier = True
        # Set when the completion-owned GVG reassert commits while HOLD still
        ...
        self.structural_observation_generation = 0
    ...
    def _route_input_generation_locked(self) -> RouteInputGeneration:
        """Capture the identity needed to discard work crossing reset HOLD."""

        graph = getattr(self, "graph", None)
        return RouteInputGeneration(
            int(getattr(self, "reset_generation", 0)),
            int(getattr(self, "request_id", 0)),
            int(getattr(self, "graph_generation", 0)),
            "" if graph is None else str(graph.graph_id),
            0 if graph is None else int(graph.revision),
        )

    def _route_input_is_current_locked(
        self, generation: RouteInputGeneration
    ) -> bool:
        """Require the captured input identity and an open HOLD barrier."""
    ...
    def _reset_reassert_is_current_locked(
        self, generation: int | None, graph
    ) -> bool:
        """Authorize only the completion-owned GVG reassert during HOLD.

        Only ``_publish_reset_completion`` passes a non-None token, so a
        truthy token plus the reassert flag set by the reset begin is enough;
        stale dispatches are still discarded by the route-input fence.
        """
        ...
        return bool(
            gvg is not None
            and bool(getattr(self, "graph_reassert_required", False))
            and self._graph_identity(graph) == self._graph_identity(gvg)
        )

    def _graph_switch_dispatch_is_current_locked(
        self,
        generation: GraphSwitchGeneration,
        graph,
        expected_input: RouteInputGeneration,
        expected_validation: CognitiveValidationGeneration | None,
        allow_reset_reassert: int | None,
    ) -> bool:
        """Final graph-service fence, called with output then state lock."""

        reset_reassert = self._reset_reassert_is_current_locked(
            allow_reset_reassert, graph
        )
        return bool(
            getattr(self, "graph_transaction_generation", None) == generation
            and self._graph_switch_request_is_current_locked(generation, graph)
            and expected_input == self._route_input_generation_locked()
            and (not self._reset_barrier_is_held() or reset_reassert)
            and (
    ...
    def _publish_structural_status_if_input_current(
        self,
        generation: RouteInputGeneration,
        state: int,
        detail: str,
    ) -> bool:
        """Publish only after the output-lock -> state-lock reset barrier."""

        with self._route_output_lock():
            with self._route_state_lock():
                if not self._route_input_is_current_locked(generation):
                    return False
            self._publish_structural_status(state, detail)
        return True

    def _publish_route_context_if_input_current(
        self, generation: RouteInputGeneration
    ) -> bool:
        """Call the context path while reset is excluded by the output lock."""
    ...
    def _publish_cognitive_constraints_if_input_current(
        self, generation: RouteInputGeneration
    ) -> bool:
        """Call the constraint path while reset is excluded by the output lock."""
    ...
    def _try_deferred_structural_rebuild(
        ...
        with self._route_state_lock():
            if (
                ...
                or (
                    expected_input is not None
                    and not self._route_input_is_current_locked(expected_input)
                )
                or getattr(self, "pending_structural_map", None) is None
        ...
        with self._route_state_lock():
            ...
            if due_s is None:
                deferred_structural = bool(
                    not self._reset_barrier_is_held()
                    and getattr(self, "pending_structural_map", None) is not None
            ...
            if due_s is not None:
                ...
                if retry_kind == "structural" and self._reset_barrier_is_held():
                    return
                reason = str(
                ...
                if (
                    self._reset_barrier_is_held()
                    and not self._graph_switch_context_is_current_reset_reassert_locked(
                        switch_context
                    )
                ):
                    # A switch retry without reset-completion authority is
                    # not consumed; it waits for the release to dispatch.
                    return
                self.graph_retry_due_steady_s = None
    ...
    def _cognitive_validation_is_current_locked(
        ...
        return bool(
            not self._reset_barrier_is_held()
            and generation == self._cognitive_validation_generation_locked()
        )
    ...
    def _on_odometry(self, message) -> None:
        ...
        with self._route_state_lock():
            if not self._route_input_is_current_locked(input_generation):
                return
            self.latest_pose_xy = pose_xy
            self.latest_pose_frame_id = frame_id
            self.latest_pose_stamp_ns = stamp_ns
    ...
    def _retire_active_route_for_reset(self):
        ...
        self.structural_observation_generation = int(
            getattr(self, "structural_observation_generation", 0)
        ) + 1
    ...
    def _on_reset_stop_gate_status(self, message) -> None:
        """Minimal hold/release tracking of the ResetStopGate authority.

        A new gate generation must begin with HOLD; the HOLD retires all
        route-era state exactly once.  Completion work runs once per
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                ...
                new_generation = seen is None or status.generation > int(seen)
                if seen is not None and status.generation < int(seen):
                    ...
                    pass
                elif new_generation and status.reason == "hold":
                ...
                elif new_generation and status.reason == "reset_complete":
                    # A late join may see completion as its first status:
                    # synchronize the already-finished generation, retire any
                    # interrupted route exactly once, and stay held until the
                    # release of this generation arrives.
                    self.reset_status_generation = status.generation
                    self.reset_status_snapshot = status
                    self.reset_intent_generation = status.generation
                    ...
                    if was_active:
                        terminal = (old_request_id, reset_epoch)
                    complete = True
                elif new_generation and status.reason.startswith("released:"):
                    if seen is None:
                        # Startup baseline: no reset of our own to reconcile.
                        self.reset_status_generation = status.generation
                        self.reset_status_snapshot = status
                    ...
                    # initialized/closed baselines establish authority while
                    # the barrier stays held.
                    self.reset_status_generation = status.generation
                    self.reset_status_snapshot = status
                    self.reset_hold_barrier = True
                    ...
                    if (
                        ...
                        if (
                            ...
                            # The volatile Empty may be absent for a late
                            # subscriber; the strict same-generation
                            # completion status owns the completion work too.
                            self.reset_event_completed_generation = (
                                status.generation
                            )
                            complete = True
                    ...
                    if (
                        ...
                        self.reset_hold_barrier = True
                        failure = (
                            "same-generation release without reset HOLD: "
                            f"generation={status.generation}, "
                            f"reason={status.reason}"
                        )
                    else:
                        self.reset_status_snapshot = status
                        # Only the release of a completed generation opens
                        # the barrier; an early release keeps it held.
                        ...
                        if (
                            ...
                            self.reset_ready_pending = False
                            # The deferred READY is only honest while the
                            # reconciled graph is still coherent; otherwise
                            # the eventual commit announces READY itself.
                            ready_pending = bool(
                                getattr(self, "graph_coherent", False)
                                and not getattr(
                                    self, "graph_reassert_required", False
                                )
                            )
    ...
    def _on_reset_event(self, _message) -> None:
        """Retire route state for one physical reset, exactly once.

        The Empty event carries no generation.  When the gate already
        announced a HOLD, the event only marks that intent complete and the
        strict status sequence owns opening the barrier.  With no gate
        authority at all (gate-less deployments), the event alone fences and
        completes the epoch and the barrier re-opens immediately.  An event
        with gate authority but no pending intent means the HOLD was lost:
        retire once and stay held (fail closed) until the status stream
        re-synchronizes.
        """
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                ...
                if intent is not None and completed != intent:
                    ...
                    # Duplicate delivery of an already-completed reset event.
                    return
                else:
                    reset = self._begin_simulation_reset_locked()
                    was_active, old_request_id, old_handle, reset_epoch = reset
                    ...
                    if getattr(self, "reset_status_authority_seen", False):
                        self.reset_hold_barrier = True
                        failure = "reset event without a pending gate HOLD"
                    else:
                        # Gate-less deployment: the Empty alone is the fence.
                        self.reset_hold_barrier = False
                        complete = True
            if terminal is not None:
                old_request_id, reset_epoch = terminal
    ...
    def _on_cognitive_graph(self, message) -> None:
        ...
        with self._route_state_lock():
            if self._reset_barrier_is_held():
                return
            input_generation = self._route_input_generation_locked()
            cognitive_mode = self.cognitive_graph_mode
        if (
            cognitive_mode in {"primary", "hybrid"}
            and not cognitive_graph_candidate_is_mature(message)
        ):
            self._publish_structural_status_if_input_current(
                input_generation,
            ...
            with self._route_output_lock():
                with self._route_state_lock():
                    if (
                        ...
                        or not self._cognitive_validation_is_current_locked(
        ...
        self._request_graph_switch(
            selected,
            f"cognitive graph selected mode={cognitive_mode} "
    ...
    def _request_graph_switch(
        ...
        try:
            ...
            with self._route_output_lock():
                with self._route_state_lock():
                    current = self._graph_switch_dispatch_is_current_locked(
                        generation, graph, expected_input,
                    ...
                    if self.graph_transaction_generation == generation:
                        ...
                        self.cognitive_graph_feedback_pending = None
                    self._schedule_graph_retry_locked(
                        "graph export invalidated" if not current
                        else f"graph export rejected: {error}",
                        switch_context=None if not current else switch_context,
                    )
                    active_graph = self.graph
                    do_fallback = bool(
                        current
            ...
            with self._route_output_lock():
                with self._route_state_lock():
                    ...
                    self._schedule_graph_retry_locked(
                        ...
                        if not current else "SetRouteGraph unavailable",
                        switch_context=None if not current else switch_context,
                    )
                    active_graph = self.graph
                    do_fallback = bool(
                ...
                self._publish_structural_status(
                    ...
                    if fallback
                    else "cognitive graph rejected: SetRouteGraph unavailable",
                )
                if feedback is not None:
                    self._publish_graph_validation(
                        replace(
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                transaction_current = (
                    self._graph_switch_dispatch_is_current_locked(
                        generation, graph, expected_input,
                        expected_validation, allow_reset_reassert,
                    )
                )
            ...
            try:
                ...
                with self._route_state_lock():
                    ...
                    self._schedule_graph_retry_locked(
                        ...
                        if not current else f"SetRouteGraph request failed: {error}",
                        switch_context=None if not current else switch_context,
                    )
                if current:
                    self._publish_structural_status(
                ...
                self._register_graph_transaction_future(
                    generation, future, "switch"
                )
    ...
    def _finish_cognitive_graph_switch(
        ...
        with self._route_state_lock():
            ...
            commit_was_reset_reassert = bool(
                commit
        ...
        if not fallback and feedback is not None:
            with self._route_output_lock():
                ...
                self._publish_structural_status(
                    self.StructuralGraphStatus.READY,
                    f"cognitive graph applied: {detail}",
                )
    ...
    def _ensure_desired_graph(
        ...
        *,
        ...
        self._request_graph_switch(
            ...
            fallback=fallback,
            expected_input=expected_input,
            allow_reset_reassert=allow_reset_reassert,
        )
    ...
    def _fallback_to_gvg_once(
        ...
        *,
            ...
            with self._route_state_lock():
                ...
                if self.primary_fallback_used:
                    ...
                    self._set_desired_graph_locked(
                        self.gvg_graph,
                        getattr(
                            self, "gvg_support", getattr(self, "support", None)
                        ),
                        require_reassert=True,
                    )
        ...
        if ensure:
            self._ensure_desired_graph(
                reason, expected_input=input_generation
            )
    ...
    def _publish_cognitive_constraints(
        ...
        if value is None:
            value = build_cognitive_constraints(
                structural_map,
                map_version=live_map_version,
                graph_revision=graph_identity[1],
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                current_region = (
                    None if self.region_selector is None
                    else self.region_selector.current
                )
                if (
                    not self._route_input_is_current_locked(input_generation)
                    ...
                    return
                if cache_miss:
                    self.cognitive_constraints_cache.misses += 1
                    self.cognitive_constraints_cache.put(cache_key, value)
                else:
                    self.cognitive_constraints_cache.hits += 1
            ...
            message.graph_id = graph_identity[0]
    ...
    def _current_xy(self) -> tuple[float, float] | None:
        ...
        with self._route_state_lock():
            if self._reset_barrier_is_held():
                return None
            odometry_xy = self.latest_pose_xy
            odometry_frame_id = self.latest_pose_frame_id
            odometry_stamp_ns = self.latest_pose_stamp_ns
        odometry_age_s = None
        if odometry_stamp_ns is not None:
            odometry_age_s = (
                self._now().nanoseconds - odometry_stamp_ns
            ) / 1.0e9
    ...
    def _on_priors(self, message) -> None:
        ...
        invalid_edges = (
            ...
            or not set(observed_ids).issubset(edge_ids)
        )
        rows = [
            (
                int(item.edge_id),
    ...
    def _on_runtime_observation(self, message) -> None:
        if not message.observed_clear and not message.planning_failed:
            return
        with self._route_state_lock():
            if self._reset_barrier_is_held():
                return
            ...
            with self._route_state_lock():
                ...
                if message.observed_clear:
                    ...
                    state = self.runtime.observe_failure(
                        edge_id, now_s,
                        occupied_ahead=bool(message.occupied_ahead),
                    )
                reroute = bool(
    ...
    def _runtime_tick(self) -> None:
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                ...
                if refresh:
                    self._arm_prior_request(now_ns)
                changed = bool(self.runtime.tick(now_ns / 1.0e9))
                priors = dict(self.latest_priors)
            if changed:
                self._publish_runtime_states()
        if retired:
    ...
    def _on_structural_map(self, message) -> None:
        with self._route_state_lock():
            ...
            input_generation = self._route_input_generation_locked()
            structural_map = self.map
            monitor = self.structural_monitor
            observation_generation = int(
                getattr(self, "structural_observation_generation", 0)
            )
            monitor_baseline = monitor.baseline.copy()
            monitor_resolution = float(monitor.resolution_m)
            monitor_settings = monitor.settings
            monitor_last_candidate = (
                None
                if monitor.last_candidate is None
                else monitor.last_candidate.copy()
            )
            monitor_first_stable_s = monitor.first_stable_s
            monitor_stable_count = int(monitor.stable_count)
        ...
        if free.shape != structural_map.free.shape:
            self.node.get_logger().warning("structural map shape does not match baseline")
            return
        now_s = self._now().nanoseconds / 1.0e9
        shadow = StructuralChangeMonitor(
            monitor_baseline, monitor_resolution, monitor_settings
        )
        shadow.last_candidate = monitor_last_candidate
        shadow.first_stable_s = monitor_first_stable_s
        shadow.stable_count = monitor_stable_count
        stable = shadow.observe(free, now_s)
        candidate_map = None
        if stable:
            candidate_map = OccupancyMap(
                ...
                resolution_m=structural_map.resolution_m,
                origin_xy_m=structural_map.origin_xy_m,
                map_version=f"{structural_map.map_version}:structural",
                yaml_path=structural_map.yaml_path,
            )
        with self._route_state_lock():
            if (
                not self._route_input_is_current_locked(input_generation)
                or self.map is not structural_map
                or self.structural_monitor is not monitor
                or int(getattr(self, "structural_observation_generation", 0))
                != observation_generation
            ):
                return
            monitor.last_candidate = shadow.last_candidate
            monitor.first_stable_s = shadow.first_stable_s
            monitor.stable_count = shadow.stable_count
            self.structural_observation_generation = observation_generation + 1
            if candidate_map is not None:
                self.pending_structural_map = candidate_map
                self._refresh_structural_intent_locked()
        if candidate_map is not None:
            self._try_deferred_structural_rebuild(expected_input=input_generation)
    ...
    def _rebuild_structural_graph(self) -> None:
        with self._route_state_lock():
            ...
            input_generation = self._route_input_generation_locked()
        if not self._publish_structural_status_if_input_current(
            input_generation,
            self.StructuralGraphStatus.REBUILDING,
            "persistent structural update",
        ):
            return
        try:
            ...
            with self._route_state_lock():
                current = self._structural_intent_is_current_locked(intent)
                if current:
                    self._schedule_graph_retry_locked(
            ...
            try:
                ...
                with self._route_state_lock():
                    ...
                    self._schedule_graph_retry_locked(
                        ...
                        if not current
                        else f"SetRouteGraph rebuild request failed: {error}",
                        kind="switch" if not current else "structural",
                    )
                if current:
                    self._publish_structural_status(
                        self.StructuralGraphStatus.LAST_KNOWN_GOOD,
    ...
    def _finish_rebuild(
        ...
        with self._route_state_lock():
            ...
            generation_current = bool(
                ...
                and (
                    ...
                    or (
                        ...
                        and generation.desired_generation
                        == int(getattr(self, "desired_graph_generation", 0))
                        and getattr(self, "pending_structural_intent", None)
                        is not None
            ...
            stale_same_epoch = bool(
                not self._reset_barrier_is_held()
                and (
                    generation is None
                    or generation.reset_generation
                    == int(getattr(self, "reset_generation", 0))
                )
            )
            commit = bool(
        ...
        if not commit:
            if stale_same_epoch:
                with self._route_state_lock():
                    stale_output_generation = self._route_input_generation_locked()
                self._publish_structural_status_if_input_current(
        ...
        with self._route_output_lock():
            with self._route_state_lock():
                if not self._route_input_is_current_locked(output_generation):
                    return
            self._publish_graph()
            self._publish_cognitive_constraints_if_input_current(output_generation)
            self._publish_structural_status(
