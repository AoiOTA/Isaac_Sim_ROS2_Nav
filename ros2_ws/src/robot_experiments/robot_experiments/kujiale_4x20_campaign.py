...
def _appearance_profile_rows() -> list[dict[str, Any]]:
    """Load the report's fixed appearance contract in display order."""

    try:
        document = yaml.safe_load(APPEARANCE_PROFILE_CONFIG.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise Campaign4x20Error(f"cannot load appearance profile config: {exc}") from exc
    profiles = document.get("profiles") if isinstance(document, Mapping) else None
    if not isinstance(profiles, Mapping):
        raise Campaign4x20Error("appearance profile config has no profiles mapping")
    rows: list[dict[str, Any]] = []
    for profile_id in ("baseline", *APPEARANCE_PROFILES):
        profile = profiles.get(profile_id)
        if not isinstance(profile, Mapping):
            raise Campaign4x20Error(f"appearance profile missing: {profile_id}")
        try:
            rows.append(
                {
                    "profile_id": profile_id,
                    "light_intensity_scale": float(profile["light_intensity_scale"]),
                    "color_temperature_k": profile["color_temperature_k"],
                    "material_hue_shift_deg": float(profile["material_hue_shift_deg"]),
                }
            )
        except (KeyError, TypeError, ValueError) as exc:
            raise Campaign4x20Error(f"invalid appearance profile: {profile_id}") from exc
    return rows
...
def _plot_trajectory_figures(summary: Mapping[str, Any], figures: Path) -> None:
    ...
    for row in summary["runs"]:
        existing_relative = row.get("trajectory_figure")
        evidence_dir = Path(str(row.get("evidence_dir", "")))
        if isinstance(existing_relative, str):
            existing = figures.parent / existing_relative
            if existing.is_file() and not evidence_dir.is_dir():
                # A legacy report refresh may retain frozen trajectory images
                # after its raw evidence has been archived elsewhere.
                continue
...
def _copy_appearance_figures(figures: Path) -> dict[str, Path]:
    """Copy the fixed living-room appearance references into a report.

    These images are reference renders for explaining the fixed experiment
    contract.  They are not substituted for per-run RGB evidence.
    """

    copied: dict[str, Path] = {}
    for profile_id, source in APPEARANCE_REPORT_SOURCE_FIGURES.items():
        if not source.is_file():
            raise Campaign4x20Error(f"appearance report figure missing: {source}")
        target = figures / source.name
        target.write_bytes(source.read_bytes())
        copied[profile_id] = target
    return copied
...
def _dashboard_asset_url(relative: str | None, image_assets: Mapping[str, str] | None) -> str:
    if not relative:
        return ""
    return image_assets.get(relative, relative) if image_assets is not None else relative
...
def _appearance_html(
    appearance_figures: Mapping[str, Path],
    image_assets: Mapping[str, str] | None,
) -> str:
    """Build the report section that makes appearance conditions inspectable."""

    profile_rows = _appearance_profile_rows()
    table_rows: list[str] = []
    cards: list[str] = []
    for row in profile_rows:
        profile_id = str(row["profile_id"])
        temperature = (
            "不覆盖" if row["color_temperature_k"] is None else f"{row['color_temperature_k']} K"
        )
        asset = _dashboard_asset_url(
            f"figures/{appearance_figures[profile_id].name}", image_assets
        )
        table_rows.append(
            "<tr>"
            f"<td><code>{html.escape(profile_id)}</code></td>"
            f"<td>{row['light_intensity_scale']:.1f}×</td>"
            f"<td>{html.escape(temperature)}</td>"
            f"<td>{row['material_hue_shift_deg']:+.0f}°</td>"
            "</tr>"
        )
        cards.append(
            f"<figure><a href='{html.escape(asset)}' target='_blank' rel='noopener'>"
            f"<img class='zoomable' src='{html.escape(asset)}' alt='{html.escape(profile_id)} 客厅外观示意图'></a>"
            f"<figcaption><strong>{html.escape(profile_id)}</strong> · {row['light_intensity_scale']:.1f}× · "
            f"{html.escape(temperature)} · 色相 {row['material_hue_shift_deg']:+.0f}°</figcaption></figure>"
        )
    return (
        "<section class='panel appearance'><h2>光照/颜色配置与客厅示意图</h2>"
        "<p>五张图均来自同一固定客厅观察位，用于直观说明本次外观变量。外观组在每轮开始前一次性应用对应匿名 USD Session Layer，"
        "全轮保持不变；不改变几何、碰撞、地图、路线或动态 actor 运动学。示意图不是逐轮前向 RGB 证据。</p>"
        "<table><thead><tr><th>配置</th><th>灯光强度</th><th>色温</th><th>材质色相偏移</th></tr></thead>"
        f"<tbody>{''.join(table_rows)}</tbody></table><div class='appearance-grid'>{''.join(cards)}</div></section>"
    )
...
def _appearance_markdown(
    ...
    table = "\n".join(
        "| `{profile_id}` | {scale:.1f}× | {temperature} | {hue:+.0f}° |".format(
            ...
            scale=row["light_intensity_scale"],
            temperature=(
                "不覆盖" if row["color_temperature_k"] is None else f"{row['color_temperature_k']} K"
            ),
            hue=row["material_hue_shift_deg"],
        )
        for row in rows
    )
    images = "".join(
    ...
    return (
        "## 光照/颜色配置与客厅示意图\n\n"
        "所有图片采用同一固定客厅观察位，直观展示本次 Session Layer 外观变量；它们不是逐轮前向 RGB 证据。"
        "每轮开始前应用一次，并保持到该轮结束；几何、碰撞、地图、路线和动态 actor 运动学均不改变。\n\n"
        "| 配置 | 灯光强度 | 色温 | 材质色相偏移 |\n"
        "| --- | ---: | ---: | ---: |\n"
        f"{table}\n"
        f"{images}\n"
...
def _write_appearance_pdf_page(
    plt: Any, pdf: Any, appearance_figures: Mapping[str, Path]
) -> None:
    """Add the fixed living-room appearance contract to the PDF report."""

    rows = _appearance_profile_rows()
    figure, axes = plt.subplots(2, 3, figsize=(16, 10), constrained_layout=True)
    figure.suptitle("光照/颜色配置与客厅示意图", fontsize=19, fontweight="bold")
    for axis, row in zip(axes.flat, rows):
        image = plt.imread(appearance_figures[str(row["profile_id"])])
        temperature = (
            "不覆盖" if row["color_temperature_k"] is None else f"{row['color_temperature_k']} K"
        )
        axis.imshow(image)
        axis.set_title(
            f"{row['profile_id']}\n{row['light_intensity_scale']:.1f}× · {temperature} · 色相 {row['material_hue_shift_deg']:+.0f}°",
            fontsize=11,
        )
        axis.axis("off")
    ...
    note_axis.text(
        ...
        0.88,
        "固定客厅观察位\n\n"
        "外观组：四种变化配置各5轮。\n每轮开始前通过匿名 USD Session Layer 应用一次，全轮不变。\n\n"
        "不改变：几何、碰撞、导航地图、路线、动态 actor 轨迹与运动学。\n\n"
        "图像用于说明实验变量，不替代逐轮 RGB 证据。",
...
def _dashboard(
    ...
    statistics_figures: Iterable[Path],
    map_figures: Iterable[Path],
    appearance_figures: Mapping[str, Path],
    *,
    ...
    appearance = _appearance_html(appearance_figures, image_assets)
    trajectory_records = [
        {
            "condition": row["condition_id"],
            "seed": row["seed"],
    ...
    demo_videos = "".join(
        f"<figure class='video-card'><figcaption>{html.escape(DEMO_VIDEOS[kind][0])}</figcaption>"
        f"<video id='demo-video-{kind}' controls preload='metadata' playsinline>"
        f"<source src='{html.escape(DEMO_VIDEOS[kind][1])}' type='video/mp4'>"
        f"<source src='{html.escape(DEMO_VIDEOS[kind][2])}' type='video/mp4'>"
        f"你的浏览器不支持视频播放；<a href='{html.escape(DEMO_VIDEOS[kind][2])}' target='_blank' rel='noopener'>打开演示视频</a>。"
        "</video><p class='media-actions'><button class='media-expand-video' type='button' "
        f"data-video-id='demo-video-{kind}'>放大视频</button>"
        f"<a href='{html.escape(DEMO_VIDEOS[kind][2])}' target='_blank' rel='noopener'>在新标签页打开视频</a></p>"
        "<p class='muted'>优先使用 GitHub attachment；若其不可用，播放器自动回退到仓库内同一视频的原始文件。</p></figure>"
    ...
    mathjax = (
        ...
        ".media-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin:10px 0 0}.media-expand-video{padding:7px 12px;border:0;border-radius:8px;background:#2563eb;color:#fff;font-weight:700;cursor:pointer}"
        ".media-modal{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;padding:20px;background:rgba(2,6,23,.9)}"
        ".media-modal[hidden]{display:none}.media-modal-card{width:min(96vw,1600px);max-height:96vh;padding:12px;border-radius:12px;background:#0f172a;box-shadow:0 24px 80px rgba(0,0,0,.5)}"
        ".media-modal-header{display:flex;align-items:center;justify-content:space-between;gap:12px;color:#fff;margin-bottom:10px}.media-modal-header button{padding:8px 13px;border:0;border-radius:8px;background:#fff;color:#0f172a;font-weight:700;cursor:pointer}"
        ".media-modal-body{display:grid;place-items:center;max-height:calc(96vh - 68px)}.media-modal-body img,.media-modal-body video{display:block;max-width:94vw;max-height:calc(96vh - 92px);width:auto;height:auto;object-fit:contain;background:#020617}</style>"
    )
    modal_markup = (
        "<div id='media-modal' class='media-modal' hidden role='dialog' aria-modal='true' aria-label='媒体放大预览'>"
        "<div class='media-modal-card' role='document'><div class='media-modal-header'><span id='media-modal-title'>媒体预览</span>"
        "<button id='media-modal-close' type='button' aria-label='关闭媒体预览'>关闭</button></div>"
        "<div id='media-modal-body' class='media-modal-body'></div></div></div>"
    )
    modal_script = """<script>(()=>{function initialize(){const modal=document.getElementById('media-modal'),body=document.getElementById('media-modal-body'),title=document.getElementById('media-modal-title'),close=document.getElementById('media-modal-close');if(!modal||!body||!close)return;function hide(){const video=body.querySelector('video');if(video)video.pause();body.replaceChildren();modal.hidden=true}function showImage(image){if(!image||!image.currentSrc&&!image.src)return;const preview=document.createElement('img');preview.src=image.currentSrc||image.src;preview.alt=image.alt||'放大图片';body.replaceChildren(preview);title.textContent=preview.alt;modal.hidden=false;close.focus()}function showVideo(video){if(!video)return;const preview=document.createElement('video');preview.controls=true;preview.preload='metadata';preview.playsInline=true;for(const source of video.querySelectorAll('source')){const copy=document.createElement('source');copy.src=source.src;copy.type=source.type;preview.appendChild(copy)}body.replaceChildren(preview);title.textContent='放大视频预览';modal.hidden=false;preview.load();close.focus()}function bindImage(image){if(!image||image.dataset.zoomBound)return;image.dataset.zoomBound='true';image.classList.add('zoomable');image.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();showImage(image)})}document.querySelectorAll('.zoomable').forEach(bindImage);bindImage(document.getElementById('trajectory-image'));document.querySelectorAll('.media-expand-video').forEach(button=>button.addEventListener('click',()=>showVideo(document.getElementById(button.dataset.videoId))));close.addEventListener('click',hide);modal.addEventListener('click',event=>{if(event.target===modal)hide()});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!modal.hidden)hide()})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initialize,{once:true});else initialize()})();</script>"""
    ...
    # The media overlay is intentionally emitted before the body content.  Its
    # initializer waits for DOMContentLoaded (or runs immediately when a saved
    # report is restored), so it binds both static images and the trajectory
    # image which is populated later by the filter script.
...
def write_4x20_report(
    ...
    with PdfPages(root / "report.pdf") as pdf:
        ...
        _write_appearance_pdf_page(plt, pdf, appearance_figures)
...
def refresh_existing_4x20_report(report_directory: str | Path) -> Path:
    """Refresh an archived report from its benchmark summary and existing figures.

    This keeps a previously generated campaign report useful after raw evidence
    has been archived: trajectory PNGs are retained, while report-owned assets,
    HTML, Markdown, PDF, and checksums are rebuilt.
    """

    root = Path(report_directory).expanduser().resolve()
    summary = _read_object(root / "benchmark.json")
    runs = summary.get("runs")
    if not isinstance(runs, list):
        raise Campaign4x20Error(f"invalid report benchmark runs: {root}")
    for row in runs:
        if not isinstance(row, dict):
            raise Campaign4x20Error(f"invalid report benchmark row: {root}")
        manifest_path = row.get("manifest_path")
        if not isinstance(manifest_path, str) or not manifest_path:
            raise Campaign4x20Error(f"report benchmark row missing manifest_path: {root}")
        row["evidence_dir"] = str(Path(manifest_path).parent)
    return write_4x20_report(summary, root, replace_output=True)
...
def main(args: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--refresh-report",
        help="refresh an existing report from its benchmark.json and retained figures",
    )
    parser.add_argument("--scope", choices=tuple(SCOPES), default="full")
    parser.add_argument("--replace-output", action="store_true")
    parsed = parser.parse_args(args)
    if parsed.refresh_report:
        if parsed.output_directory or parsed.status:
            parser.error("--refresh-report cannot be combined with --output-directory or --status")
        print(refresh_existing_4x20_report(parsed.refresh_report))
        return
    if not parsed.run_root:
        parser.error("--run-root is required unless --refresh-report is used")
