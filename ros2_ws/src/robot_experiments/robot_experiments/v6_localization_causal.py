...
def route_actions(config: LocalizationConfig, arm: str) -> tuple[dict[str, Any], ...]:
    ...
    return (
        {"action": "goal", "leg_id": "G2"},
        {
            ...
            "kind": "deterministic_initialpose" if config.selected_variant else config.fault_kind,
            "service": None if config.selected_variant else config.fault_service,
        },
...
class LocalizationCausalNode(V6FormalNode):
    ...
    def _fault(self) -> None:
        ...
        self._event(
            "fault_injected",
            fault_id=self.config.fault_id,
            kind=("deterministic_initialpose" if whole_house else self.config.fault_kind),
