...
def _attempt28_candidate(name):
    return yaml.safe_load(
        (PACKAGE_ROOT / 'config' /
         f'nav2_attempt28_{name}.yaml').read_text())
...
def test_attempt28_a1_and_a19_profiles_are_explicitly_isolated():
    stock = _attempt28_candidate('no_candidate')
    assert stock == {'controller_server': {'ros__parameters': {}}}
    ...
    for name, (tolerance, batch) in expected.items():
        ...
        assert parameters['progress_checker_plugins'] == ['progress_checker']
        assert parameters['goal_checker_plugins'] == [
            'goal_checker', 'a19_crossing_goal_checker']
...
def test_attempt28_uses_one_frozen_unpadded_footprint_and_padding():
    config = _config()
    expected = (
        '[[0.255, 0.210], [0.255, -0.210], '
        '[-0.230, -0.210], [-0.230, 0.210]]')
    local = config['local_costmap']['local_costmap']['ros__parameters']
    global_costmap = config['global_costmap']['global_costmap'][
        'ros__parameters']
    assert local['footprint'] == expected
    assert global_costmap['footprint'] == expected
    assert local['footprint_padding'] == 0.005
    assert global_costmap['footprint_padding'] == 0.005


def test_attempt28_launch_exposes_distinct_cycle_trace_identity():
    source = (PACKAGE_ROOT / 'launch' / 'navigation.launch.py').read_text()
    assert 'attempt28_cycle_trace_library' in source
    assert 'attempt28_cycle_trace_output' in source
    assert 'BIO_NAV_ATTEMPT28_CYCLE_TRACE' in source
