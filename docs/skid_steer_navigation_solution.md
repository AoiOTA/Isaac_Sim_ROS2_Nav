...
> 文档状态：历史专项复盘。当前手动导航的启动命令、地图默认值和 MPPI 参数以 [`user_manual.md`](user_manual.md)、[`interfaces.md`](interfaces.md) 与 `ros2_ws/src/robot_navigation/config/nav2_params.yaml` 为准；不要把本文件中的旧批次命令当作当前入口。
