...
嵌入本文件的测试地图，并在实验设计之后加入“光照/颜色配置与客厅示意图”：同一固定客厅机位展示 `baseline`、`dim_warm`、`dim_cool`、`bright_warm`、`bright_cool`，逐项列出灯光强度倍率、色温和材质色相偏移。HTML、便携 HTML、Markdown 和 PDF 均包含该区；示意图说明实验变量，不替代逐轮前向 RGB 证据。支持按实验组、seed、外观配置、动态变体和结果筛选。每个有
...
静态/动态 2×20 子报告只显示本范围的两张条件地图；完整4×20总报告拆成“静态两组对比”和“动态两组对比”，不再将四张地图缩放到同一画布。报告首先以 S1–D2 中文卡片解释四组分别控制什么：静态或三阶段动态障碍、基准外观或四种外观配置，以及20轮如何分配。地图、统计图、客厅外观图和可筛选的逐轮 GT 路径均可点击，在当前页弹窗放大；避障演示视频使用“放大视频”按钮打开大尺寸播放器。所有媒体保留新标签页打开原文件的后备入口。图片快照已发布到 `docs/report_assets/<report-dir>/` 的报告，会在 `index.html`、`index_portable.html` 和 `report.md` 中使用 GitHub Raw 链接；复制 HTML 到另一台电脑后只需网络访问 GitHub，不需同传 PNG。未发布快照时自动回退为相对 PNG 路径，避免将不存在的外链写入报告。报告摘要之后嵌入对应的4×速避障演示视频（静态/动态子报告各一段，完整报告两段）；指定 GitHub attachment 不可用时自动回退到仓库内的同一 MP4。可筛选的“逐轮实际 GT 路径”置于统计图之前。
