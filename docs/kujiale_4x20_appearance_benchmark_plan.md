...
静态/动态 2×20 子报告只显示本范围的两张条件地图；完整4×20总报告拆成“静态两组对比”和“动态两组对比”，不再将四张地图缩放到同一画布。每张地图与统计图均可点击打开原尺寸。除目录内的轻量 `index.html` 外，报告同时生成 `index_portable.html`：将地图、统计图及所有逐轮 GT 轨迹以 Base64 内嵌，复制这一文件到另一台电脑即可离线预览图片和使用轨迹筛选，无需同传任何 PNG。报告摘要之后嵌入对应的4×速避障演示视频（静态/动态子报告各一段，完整报告两段）；指定 GitHub attachment 不可用时自动回退到仓库内的同一 MP4。可筛选的“逐轮实际 GT 路径”置于统计图之前，且路径图也支持打开原尺寸。
