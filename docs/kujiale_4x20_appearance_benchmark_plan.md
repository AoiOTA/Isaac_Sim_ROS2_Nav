...
并保留基准/外观变化的同seed配对记录。完整报告的 HTML、Markdown 和 PDF 都应在结果前写明实验如何执行及当次指标结果：物理避障率、静态相对理论最短可行路径偏差、以及分静态/动态/总体的完整任务导航成功率。即使未达到门槛也必须生成报告；验收通过返回0，批次完成但门槛或证据失败返回2。
