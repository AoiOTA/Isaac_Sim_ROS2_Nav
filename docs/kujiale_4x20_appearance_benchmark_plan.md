...
对外统计的名称、分子分母和边界以[通用指标定义与对外表述](kujiale_4x20_metric_definitions.md)为准：静态/动态避障成功率分别采用 `ASR_s=N_s^succ/N_s`、`ASR_d=N_d^succ/N_d`，导航成功率采用 `NSR=N_goal/N`；静态路径偏差只与同约束的静态理论最短可行参考比较，动态场景不套用该固定参考。
...
静态/动态 2×20 子报告只显示本范围的两张条件地图；完整4×20总报告拆成“静态两组对比”和“动态两组对比”，不再将四张地图缩放到同一画布。每张地图与统计图均可点击打开原尺寸。报告摘要之后嵌入对应的4×速避障演示视频（静态/动态子报告各一段，完整报告两段）；指定 GitHub attachment 不可用时自动回退到仓库内的同一 MP4。可筛选的“逐轮实际 GT 路径”置于统计图之前，且路径图也支持打开原尺寸。

报告展示四组成功率和置信区间、耗时分布、路径长度、静态偏差、恢复次数、碰撞、动态交互有效性和失败原因，
并保留基准/外观变化的同seed配对记录。完整报告的 HTML、Markdown 和 PDF 都应在结果前写明实验如何执行及当次指标结果：静态/动态避障成功率公式、静态相对理论最短可行路径偏差公式，以及分静态/动态/总体的导航成功率公式。即使未达到门槛也必须生成报告；验收通过返回0，批次完成但门槛或证据失败返回2。
