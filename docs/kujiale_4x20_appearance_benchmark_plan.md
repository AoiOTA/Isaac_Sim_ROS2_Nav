...
静态/动态 2×20 子报告只显示本范围的两张条件地图；完整4×20总报告拆成“静态两组对比”和“动态两组对比”，不再将四张地图缩放到同一画布。报告首先以 S1–D2 中文卡片解释四组分别控制什么：静态或三阶段动态障碍、基准外观或四种外观配置，以及20轮如何分配。地图、统计图、客厅外观图和可筛选的逐轮 GT 路径均可点击，在当前页弹窗放大。除目录内的轻量 `index.html` 外，报告同时生成 `index_portable.html`：将地图、统计图、五张客厅外观图及所有逐轮 GT 轨迹以 Base64 内嵌，复制这一文件到另一台电脑即可离线预览图片和使用轨迹筛选，无需同传任何 PNG，也不依赖浏览器打开大型 data URL。报告摘要之后嵌入对应的4×速避障演示视频（静态/动态子报告各一段，完整报告两段）；指定 GitHub attachment 不可用时自动回退到仓库内的同一 MP4。可筛选的“逐轮实际 GT 路径”置于统计图之前。
