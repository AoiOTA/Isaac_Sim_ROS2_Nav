...
静态/动态 2×20 子报告只显示本范围的两张条件地图；完整4×20总报告拆成“静态两组对比”和“动态两组对比”，不再将四张地图缩放到同一画布。报告首先以 S1–D2 中文卡片解释四组分别控制什么：静态或三阶段动态障碍、基准外观或四种外观配置，以及20轮如何分配。地图、统计图、客厅外观图和可筛选的逐轮 GT 路径均可点击，在当前页弹窗放大。图片快照已发布到 `docs/report_assets/<report-dir>/` 的报告，会在 `index.html`、`index_portable.html` 和 `report.md` 中使用 GitHub Raw 链接；复制 HTML 到另一台电脑后只需网络访问 GitHub，不需同传 PNG。未发布快照时自动回退为相对 PNG 路径，避免将不存在的外链写入报告。报告摘要之后嵌入对应的4×速避障演示视频（静态/动态子报告各一段，完整报告两段）；指定 GitHub attachment 不可用时自动回退到仓库内的同一 MP4。可筛选的“逐轮实际 GT 路径”置于统计图之前。
