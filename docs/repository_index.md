...
| `scripts/run_contact_ab_matrix.sh` | 接触/ground-topology 严格串行 A/B 入口；在默认 `--environment all` 下，`--ground-topology baseline` 保持 SimplePlane-only1 + Warehouse-combined32 的 36-run/12-group 历史口径，`all` 展开三个合法 environment/topology pair 的 54-run/18-group 矩阵，显式 ID 必须匹配单一环境。默认使用项目 robot，也可用 `--robot-config FILE` 选择仓库内 canonical/tracked/与 HEAD blob 原始字节一致的版本化候选；robot、topology 和六个 contact profile 都进入跨轮 HEAD/hash 锁。脚本清除继承的嵌套覆盖，只恢复 project/topology/contact/robot，逐轮验证 schema-v5 topology/contact canonical JSON、Stage readback、target 交叉门、完整 kinematics、六段/轮向报告，并写入 43 列 manifest。最终要求 analysis schema 3/physical-acceptance schema 1 并生成 batch-summary schema 4；summary `result=success` 只表示证据闭合，物理结果另记 `all_applicable_groups_passed` 与 applicable/not-applicable/passing/failed 四类 group，N/A 不计失败。脚本仍绑定冻结 manifest/analysis SHA，清理只信号本会话认证进程组。历史 `d5840ed` 工件为 analysis 2/summary 3，不能回填新字段或写成 `0/12 fail`。输出结构和阅读顺序见使用手册第 17.5 节。 |
