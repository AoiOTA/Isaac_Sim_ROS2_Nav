...
> 文档状态：原一期设计已完成，保留最初仅 Local Costmap 融合的设计过程。**当前代码按 Nav2 profile 分流**：`stable` 将深度点云接入全局和局部 `depth_voxel_layer`，`dynamic_avoidance` 仅在 Local STVL 使用深度点云以清除移动 actor 残影；Collision Monitor 始终仅订阅 `/scan`。4×20光照/颜色 campaign 已实现但尚未执行80轮，不应把早期长路线结果写成其验收结论。可执行配置以 `ros2_ws/src/robot_navigation/config/nav2_params.yaml`、[`interfaces.md`](interfaces.md)、[`user_manual.md`](user_manual.md) 与 [`kujiale_4x20_appearance_benchmark_plan.md`](kujiale_4x20_appearance_benchmark_plan.md) 为准。
>
