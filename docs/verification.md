...
中心区四组可手调静态障碍和两组动态障碍为新布局，尚未验收；生成报告存放在被忽略的 `data/reports/`。
...
| `warehouse_new` narrow-passage profile | Current configuration retains the physical `0.485 x 0.420 m` footprint, `0.005 m` padding and `0.40 m / 8.0` inflation; StopZone remains `0.535 x 0.460 m`. The 2026-07-23 smoothness tuning uses a `0.660 x 0.470 m` SlowdownZone at 90%, requires 5 scan points, disables predictive ApproachZone, fixes MPPI noise and uses the 500-sample stable overlay. | The earlier `49.09 s → 27.87 s` result was obtained with the previous `0.770 x 0.470 m`/85%/ApproachZone profile and remains historical evidence only. The current smoothness tuning has passed configuration tests but needs a new GUI navigation run before any performance claim. |
