...
HTML 报告在分组摘要后首先展示“实验设计与指标口径”：包含固定地图/路线、四组20轮、静态六障碍、三阶段 actor、外观扰动与重启隔离方法，以及本报告范围的静态/动态物理避障率、静态路径偏差和导航成功率。相同内容同步写入 `report.md` 和 `report.pdf` 首页。随后提供对应的避障演示视频：静态子报告播放静态演示，动态子报告播放动态演示，完整4×20报告同时提供两段视频。视频优先使用指定的 GitHub attachment；播放器还配置了仓库内同一 MP4 的原始文件作为回退，因此 attachment 暂不可达时仍可在页面内直接播放。
...
如需将结果写入交付材料，请使用[指标定义与对外表述口径](kujiale_4x20_metric_definitions.md)。其中“物理避障率”只描述真实接触安全，“导航成功率”还要求全航点和动态交互完成；不要以其中一项替代另一项。
