...
测试地图不再把四组缩在一张图内：完整报告分为“静态两组对比”和“动态两组对比”，每次只并列两张；静态/动态 2×20 子报告只显示本范围的一对地图。地图、统计图和逐轮 GT 路径均可点击，在当前页弹窗放大；已发布快照的 HTML 使用可直接打开的 GitHub Raw 图片链接，不再生成大型 Base64 内容。报告首页的 S1–D2 中文卡片同时写明每组控制变量：静态/动态障碍类型、外观是否改变，以及每组20轮的具体分配。
