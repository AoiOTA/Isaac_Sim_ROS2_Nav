...
HTML 报告在分组摘要后首先展示“实验设计与指标口径”：包含固定地图/路线、四组20轮、静态六障碍、三阶段 actor、外观扰动与重启隔离方法，以及通用公式 `ASR_s=N_s^succ/N_s`、`ASR_d=N_d^succ/N_d`、`δ_i=(L_i-L_i*)/L_i*` 和 `NSR=N_goal/N`。相同内容同步写入 `report.md` 和 `report.pdf` 首页。随后提供对应的避障演示视频：静态子报告播放静态演示，动态子报告播放动态演示，完整4×20报告同时提供两段视频。视频优先使用指定的 GitHub attachment；播放器还配置了仓库内同一 MP4 的原始文件作为回退，因此 attachment 暂不可达时仍可在页面内直接播放。
