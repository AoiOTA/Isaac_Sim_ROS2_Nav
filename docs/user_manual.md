...
HTML 报告的“逐轮实际 GT 路径”区会把每轮 `ground_truth.csv.gz` 叠加至 `warehouse_new` 地图；条件、seed、外观和结果
筛选会同步缩小可选路径。静态路径图会额外绘制正式静态场景的六个橙色 RGB-D 障碍物。动态路径图会从本轮
`dynamic_obstacles.csv.gz` 绘制实际触发 actor 的彩色虚线轨迹、起点方块、终点X和方向箭头；未触发且始终
处于 `waiting` 的 actor 不显示。若升级报告器后只需重绘既有报告而不改动原始证据，可在对应报告命令末尾加 `--replace`。
