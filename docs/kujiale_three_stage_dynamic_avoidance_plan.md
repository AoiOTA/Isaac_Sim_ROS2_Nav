...
触发门设为 `y=2.60`，让机器人在保持窄通道对准后更接近同向 actor 才释放运动，避免出现仅远距离并行而未形成跟随交互。横向窗口、南向速度门仍要求机器人已经对准狭窄通道，避免在 G2 出发转向阶段提前触发。验收时，运行器仅把 actor 运动中的同车道前方距离 `0.20–1.35 m` 计为跟随；上限 `1.35 m` 来自校准试运行中连续记录的最小 `1.2301 m` 前车距，仍会排除远距离并行。actor 的余弦缓入缓出运动时长约为 `4.11 s`，不会退化为静态突然封堵。
...
| `dynamic_avoidance` | `15 Hz`、30 步、`1/15 s`、500 条采样 | `1.20 m/s` / `3.40 rad/s` | `60 Hz` | Local STVL（10 Hz 更新、5 Hz 发布）+ `0.50 m` 局部膨胀；Global 仅静态图 + LiDAR。 |
