...
```text
G1->G2   (-1.65, -0.20) -----> (-1.30, -0.20)  横向移动并停车
...
| actor 轨迹 | `[-1.65, -0.20, 0.50] -> [-1.30, -0.20, 0.50]` |
| 峰值速度 / 最大加速度 | `0.80 m/s` / `1.60 m/s^2` |
| 预期行为 | 机器人从 actor 右侧绕过，继续进入原定左侧狭窄通道并到达 G2。 |

这是已经调通的基线，不改变当前 MPPI、Velocity Smoother 或 RViz 候选轨迹下采样参数。为满足 `warehouse_new` 5 cm 栅格中的 `>=0.05 m` 墙体余量，轨迹较原 `y=-0.15` 的视觉基线向下修正 `0.05 m`；停车点保持在左侧 `x=-1.30`，使机器人从右侧通过时仍能满足 `0.10 m` 最小净距，避免 actor 在通道中央安全让行。
