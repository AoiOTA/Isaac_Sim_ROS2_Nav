...
- 正式四类障碍的巡航速度限定在 `0.25–0.50 m/s`；局部绕障调优 case 单独采用 `0.80 m/s` 的短程横移。
...
- 触发：机器人在 `x∈[-0.70, 0.30]` 内北向通过门口 `y=-1.75` 后 actor 开始横移；它采用峰值 `0.80 m/s`、加速度不超过 `1.60 m/s²` 的余弦缓入缓出曲线，约 `1.57 s` 后到达预先验证的绕行点并主动 `parked`。
- 轨迹：`[-1.65, -0.15] -> [-0.85, -0.15]`。actor 从机器人左侧横向向右移动，完整方块保持在预检自由带内，并在左侧通路入口旁停车；右侧保留连续绕行空间，actor 不会进入小车的回转出口。
- 尺寸/速度：`0.40 × 0.40 × 1.00 m`，峰值 `0.80 m/s`。
